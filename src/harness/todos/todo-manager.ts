export const TODO_STATUSES = ["pending", "running", "completed", "failed"] as const;
...
export class TodoManager {
  ...
  formatForPrompt(): string {
    if (this.todos.length === 0) return "当前没有计划步骤。";
    return this.todos.map((todo) => `[${todo.status}] #${todo.id} ${todo.description}`).join("\n");
  }
