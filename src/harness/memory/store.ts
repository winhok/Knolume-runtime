...
export class MemoryStore {
  ...
  prepareLint(): PreparedMemoryLintEntry[] {
    return prepareMemoryLint(this.list());
  }
  ...
  buildPromptSection(): string {
    ...
    const lines = [
      ...
      "",
      "记忆使用原则：",
      "- 记忆是线索，不是事实——使用前先用工具验证（使用宿主提供的工具确认引用内容是否仍然有效）",
  ...
  private createFilename(type: MemoryEntry["type"], name: string, slug: string): string {
    const hash = createHash("sha256").update(`${type}\0${name}`).digest("hex").slice(0, 12);
  ...
  private assertWithinMemory(memoryRoot: string, filePath: string, filename: string): void {
