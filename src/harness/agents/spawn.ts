...
export interface SpawnContext {
  ...
  projectRules?: string;
  runtimePromptSections?: Array<{ requiredTool: string; text: string }>;
}
...
function buildSubAgentSystem(
  ...
  const activeToolNames = Object.keys(toolRuntime.getTools(selection));
  const activeTools = activeToolNames.join(", ");
  const activeToolSet = new Set(activeToolNames);
  const deferred = toolRuntime.getDeferredToolSummary?.(selection);
  return [
    ...
    ...runtimePromptSections
      .filter((section) => activeToolSet.has(section.requiredTool))
  ...
  try {
    ...
    const result = await agentLoop({
      ...
      system: buildSubAgentSystem(
        ...
        toolRuntime,
        resolved.selection,
        ctx.executionContextDescription,
        ctx.projectRules,
        ctx.runtimePromptSections,
      ),
    ...
    const errorMessage = parentCancelled
      ...
      : isAbort
        ...
        : error instanceof Error
          ? error.message
          : String(error);
    if (parentCancelled) ctx.agentRegistry.cancel(runId, errorMessage);
    else ctx.agentRegistry.fail(runId, errorMessage, isAbort);
    await trace?.finish(isAbort ? "cancelled" : "failed", error);
    console.log(
      `  ${tag} ${parentCancelled ? "取消" : isAbort ? "超时" : "失败"} ✗: ${errorMessage}`,
    );
    if (parentCancelled) {
      throw controller.signal.reason instanceof Error
        ? controller.signal.reason
        : new DOMException("Parent run aborted", "AbortError");
    }
