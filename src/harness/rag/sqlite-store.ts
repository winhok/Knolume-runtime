...
export class SqliteVectorStore {
  ...
  constructor(dbPath: string = "knowledge.db", options: SqliteVectorStoreOptions = {}) {
  ...
  replaceSource(
    ...
    const replace = this.db.transaction(() => {
      if (this.vectorExtensionAvailable) {
        ...
        const deleteVector = this.db.prepare("DELETE FROM chunks_vec WHERE id = ?");
  ...
  clear(): void {
    this.db.exec("DELETE FROM chunks; DELETE FROM chunks_fts; DELETE FROM documents;");
    if (this.vectorExtensionAvailable) {
  ...
  sources(): string[] {
    const rows = this.db.prepare("SELECT DISTINCT source FROM chunks").all() as Array<{
      source: string;
    }>;
