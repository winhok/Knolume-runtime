...
async function runAgentLoopCore({
  ...
  try {
    while (step < maxSteps) {
      ...
      for (let attempt = 1; ; attempt++) {
        try {
          const result = streamText({
            model,
            system,
            tools: toolRuntime.getTools(toolSelection),
      ...
      totalUsage.inputTokens += normalizedUsage.inputTokens;
      totalUsage.outputTokens += normalizedUsage.outputTokens;
      totalUsage.cacheReadTokens += normalizedUsage.cacheReadTokens;
      totalUsage.cacheWriteTokens += normalizedUsage.cacheWriteTokens;
      ...
      if (
        stepRecord?.cost !== undefined &&
        stepRecord.currency !== undefined &&
