...
function getTokenPricing(model: string, usage: StepUsage): TokenPricingTier {
  ...
  const tier = pricing.tiers.find(
    (candidate) =>
      candidate.upToInputTokens === undefined || inputTokens <= candidate.upToInputTokens,
  );
...
export class UsageTracker implements UsageRecorder {
  ...
  record(model: string, usage: StepUsage): StepRecord {
    ...
    if (this.currency && this.currency !== pricing.currency) {
      throw new Error(`Cannot mix ${this.currency} and ${pricing.currency} pricing in one tracker`);
    }
  ...
  totals() {
    const t = this.steps.reduce(
      (a, s) => ({
        ...
        cacheStorageTokenHours: a.cacheStorageTokenHours + (s.cacheStorageTokenHours ?? 0),
        cost: a.cost + s.cost,
      }),
      {
        inputTokens: 0,
...
export function computeCost(model: string, usage: StepUsage): number {
  ...
  const tokenCost =
    ...
    1_000_000;
  const storageCost =
    ((usage.cacheStorageTokenHours ?? 0) * (pricing.cacheStorage ?? 0)) / 1_000_000;
  return tokenCost + storageCost;
}

export function normalizeUsage(usage: LanguageModelUsage | null | undefined): StepUsage {
  if (!usage)
    return {
      inputTokens: 0,
      outputTokens: 0,
  ...
  const inputTokens =
    usage.inputTokenDetails.noCacheTokens ?? Math.max(0, rawInputTokens - cacheRead - cacheWrite);
