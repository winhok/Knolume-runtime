...
export function microcompact(
  ...
  clearableTools: ReadonlySet<string> = new Set(),
): {
  messages: ModelMessage[];
  cleared: number;
} {
  ...
  const result = messages.map((msg, idx) => {
    ...
    if (!clearableTools.has(toolName)) return msg;
  ...
  return message.content
    .map((rawPart) => {
      ...
      if (part.type === "tool-call") {
        return `[tool-call name=${safeIdentifier(part.toolName)} toolCallId=${safeIdentifier(part.toolCallId)}] ${JSON.stringify(part.input ?? {})}`;
      }
      if (part.type === "tool-result" && "output" in part) {
        return `[tool-result name=${safeIdentifier(part.toolName)} toolCallId=${safeIdentifier(part.toolCallId)}] ${toolResultOutputToText(part.output as Parameters<typeof toolResultOutputToText>[0])}`;
      }
      return "";
    })
    .filter(Boolean)
...
export async function summarize(
  ...
  if (tokenEstimate < thresholdTokens || messages.length <= keepRecentMessages) {
