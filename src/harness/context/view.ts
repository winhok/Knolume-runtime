...
export interface ContextSnapshot {
  ...
  modelId: string | null;
  /** Provider-advertised nominal window. Null when the provider does not expose it. */
...
export function renderContextMatrix(snapshot: ContextSnapshot): string {
  const { windowTokens, slices, autocompactReserveTokens, safetyReserveTokens } = snapshot;
  const TOTAL_CELLS = 256;
  const matrixWindowTokens = windowTokens ?? snapshot.effectiveWindowTokens;
  const tokensPerCell = matrixWindowTokens / TOTAL_CELLS;
  const cells: number[] = [];

  for (const slice of slices) {
    if (slice.tokens <= 0) continue;
    const sliceCells = Math.max(1, Math.round(slice.tokens / tokensPerCell));
    for (let index = 0; index < sliceCells && cells.length < TOTAL_CELLS; index++) {
      cells.push(slice.color);
    }
  }

  const bufferCells = Math.max(0, Math.round(autocompactReserveTokens / tokensPerCell));
  const safetyCells = Math.max(0, Math.round((safetyReserveTokens ?? 0) / tokensPerCell));
  const freeCells = Math.max(0, TOTAL_CELLS - cells.length - bufferCells - safetyCells);
  for (let index = 0; index < freeCells; index++) cells.push(-1);
  for (let index = 0; index < bufferCells && cells.length < TOTAL_CELLS; index++) {
    cells.push(-2);
  }
  for (let index = 0; index < safetyCells && cells.length < TOTAL_CELLS; index++) {
    cells.push(-3);
  }
...
export function renderContextLegend(snapshot: ContextSnapshot): string {
  ...
  lines.push(`\x1b[1m${fg(COLORS.text, snapshot.modelName ?? "Model provider unknown")}\x1b[0m`);
  lines.push(fg(COLORS.dim, snapshot.modelId ?? "Provider model id unknown"));
  lines.push(
    `${fmtTokens(usedTokens)}/${fmtTokens(effectiveWindowTokens)} effective tokens (${pct(usedTokens, effectiveWindowTokens)})`,
  );
  lines.push(
    windowTokens === null
      ? fg(COLORS.dim, "Nominal provider window: unknown")
      : `Nominal window: ${fmtTokens(windowTokens)} tokens`,
  );
  ...
  for (const slice of slices) {
    ...
    const value = `${fmtTokens(slice.tokens)} tokens (${pct(slice.tokens, windowTokens ?? effectiveWindowTokens)})`;
    lines.push(`${dot} ${label}: ${value}`);
  }

  const autocompactThresholdTokens = effectiveWindowTokens - autocompactReserveTokens;
  const free = Math.max(0, autocompactThresholdTokens - usedTokens);
  lines.push(
    `${fg(COLORS.free, "○")}  Free before autocompact: ${fmtTokens(free)} (${pct(free, windowTokens ?? effectiveWindowTokens)})`,
  );
  lines.push(
    `${fg(COLORS.buffer, "▢")}  Autocompact reserve: ${fmtTokens(autocompactReserveTokens)} (${pct(autocompactReserveTokens, windowTokens ?? effectiveWindowTokens)})`,
  );
  lines.push(
    safetyReserveTokens === null
      ? fg(COLORS.dim, "▧  Safety reserve: unknown without provider window metadata")
      : `${fg(COLORS.safety, "▧")}  Safety reserve: ${fmtTokens(safetyReserveTokens)} (${pct(safetyReserveTokens, windowTokens ?? effectiveWindowTokens)})`,
  );
  return lines.join("\n");
}
...
export function buildContextSnapshot(input: BuildSnapshotInput): ContextSnapshot {
  ...
  const estimatedBreakdownTokens = slices.reduce((total, slice) => total + slice.tokens, 0);
  const measuredTokens =
    input.tokenMeasurement.observedPromptTokens === null
      ? estimatedBreakdownTokens
      : input.tokenMeasurement.observedPromptTokens + input.tokenMeasurement.pendingEstimatedTokens;
  ...
  return {
    ...
    safetyReserveTokens:
      input.windowTokens === null
        ? null
        : Math.max(0, input.windowTokens - input.effectiveWindowTokens),
...
export function renderUsageView(tracker: UsageTracker): string {
  ...
  const totalCacheable = totals.cacheReadTokens + totals.cacheWriteTokens + totals.inputTokens;

  lines.push(bold(color(255, "  Usage Summary")));
  lines.push(color(244, `  ${totals.steps} 步累计 · ${fmtLocalTimestamp(new Date())}`));
  lines.push("");
  lines.push(
  ...
  const bar = color(36, "█".repeat(filled)) + color(240, "░".repeat(barWidth - filled));
  lines.push(`  Cache hit rate  ${bar}  ${(totals.hitRate * 100).toFixed(1)}%`);
  lines.push("");
  lines.push(`  ${bold("Cost")}            ${color(220, `${currency}${totals.cost.toFixed(4)}`)}`);
  lines.push(
    `  ${color(244, "Without cache")}   ${color(244, `${currency}${totals.baselineCost.toFixed(4)}`)}`,
  );
  const savedPercent = totals.baselineCost > 0 ? (totals.savedCost / totals.baselineCost) * 100 : 0;
