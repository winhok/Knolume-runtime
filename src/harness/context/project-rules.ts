...
const MAX_RULE_SECTION_CHARS = 48 * 1024;
    ...
    for (const filename of RULE_FILES) {
      ...
      try {
        ...
        try {
          ...
          if (!stats.isFile()) throw new Error("规则路径不是普通文件");
          if (stats.size > MAX_RULE_FILE_BYTES) {
            throw new Error(`规则文件超过 ${MAX_RULE_FILE_BYTES} 字节限制`);
          }
...
export function formatProjectRules(rules: readonly ProjectRule[]): string {
  ...
  if (section.length > MAX_RULE_SECTION_CHARS) {
    throw new Error(`项目规则超过 ${MAX_RULE_SECTION_CHARS} 字符限制`);
  }
  return section;
}
